...
from mysite.settings import SITE_URL
User = get_user_model()
...
def profileview(request):
	...
	if user.is_authenticated:
		fgaccount=SocialAccount.objects.filter(user=user)	
		if fgaccount:		
			try:	
				f=fgaccount.get(provider='facebook')
			except:
				f=None
			if f:
				...
				try:
					...
					old_user=None
				if old_user:
					if old_user.provider!='facebook':
						provider=old_user.provider
						return HttpResponse("sorry this email_id is already registered with %s,so please login with %s" %provider %provider)
					print('facebook')
				else:					
					newuser = SUserProfile(
							  susername=fusername,
							  email_id=femail,
					...
					print('else')
					newuser.save()
				return render_to_response('account/facebookprofile.html',locals(),context_instance=RequestContext(request))
	
			try:
				g=fgaccount.get(provider='google')
			except:
				g=None
			if g:
				...
				gemail=gdata.get('email')
				gprovider=g.provider
				try: 
					print('pepe')
					old_user=SUserProfile.objects.get(email_id=gemail)
				except:
					old_user=None
					print('except')
				if old_user:
					if old_user.provider!='google':
						provider=old_user.provider
						return HttpResponse('sorry this email_id is already registered with {},so please login with {}'.format(provider,provider))
				else:
					print('kele')
					newuser = SUserProfile(
							  susername=gusername,				
					...
					newuser.save()
				return render_to_response('account/googleprofile.html',locals(),context_instance=RequestContext(request))
					
			#try:	
			#	t=fgaccount.get(provider='twitter')	
			#except:
			#	t=None
			#if t:
				#tusername=t.get_provider_account
				#tdata=t.extra_data
				#temail=tdata['email'],twitter api is not giving access to email-id,so avoiding it.
				#tname=tdata['name']
				#try: 
				#	old_user=SUserProfile.objects.get(susername=tusername)
				#except:
				#	newuser=SUserProfile(
				#	susername=tusername,
				#	email_id=temail,
			...
			return redirect('/original/login/')
	return render_to_response('account/profile.html',locals(),context_instance=RequestContext(request))
